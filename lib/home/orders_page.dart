...
class OrdersPage extends StatefulWidget {
  ...
  static Future<List<Map<String, dynamic>>> fetchOrders() async {
    final response = await http.get(Uri.parse('http://localhost:3001/orders'));
...
class _OrdersPageState extends State<OrdersPage> {
  ...
  Future<void> _deleteOrder(String orderId) async {
    final response =
        await http.delete(Uri.parse('http://localhost:3001/orders/$orderId'));
...
class OrderCard extends StatelessWidget {
  ...
  const OrderCard({super.key, required this.orderItem, required this.onDelete});
